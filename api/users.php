<?php
	...
	function createUser() {
		...
		$response = uploadFileCv("file_user_cv");
		if ($response["status"]) {
			...
			if(mysqli_query($conn, $query)) {
				$response["status"] = 1;
				$response["status_message"] = 'User created successfully.';
			} else {
				$response["status"] = 0;
				$response["status_message"] = 'Error creating user. '. mysqli_error($conn);
			}
		}
		header('Content-Type: application/json');
	...
	function updateUserUpload($id) {
		...
		if ($response["status"]) {
			...
			if(mysqli_query($conn, $query)) {
				$response["status"] = 1;
				$response["status_message"] = 'User updated successfully.';
			} else {
				$response["status"] = 0;
				$response["status_message"] = 'Error updating user. '. mysqli_error($conn);
			}
		}
		header('Content-Type: application/json');
		echo json_encode($response);
	...
	switch($request_method) {
		...
		case 'POST':
			// Ajouter un produit
			if(isset($_POST["callType"]) &&( $_POST["callType"] == "register")) {
				register();
			} elseif(isset($_POST["callType"]) && ($_POST["callType"] == "login")) {
				login();
			} elseif(isset($_POST["callType"]) && ($_POST["callType"] == "updateProfileUpload")) {
				$id=intval($_GET["id"]);
				updateProfileUpload($id);
			} elseif(isset($_POST["callType"]) && ($_POST["callType"] == "updateUserUpload")) {
				$id=intval($_GET["id"]);
				updateUserUpload($id);
			} else {
				createUser();
